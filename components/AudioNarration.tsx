...
const AudioNarration: React.FC<AudioNarrationProps> = ({ text, autoPlay = false }) => {
  ...
  return (
    <div className="flex items-center space-x-4 bg-slate-100 p-3 rounded-full border border-slate-200 shadow-sm">
      <button
        onClick={playAudio}
        disabled={isLoading}
      ...
      <div className="flex items-center space-x-2 flex-grow pr-2">
      ...
      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">智慧語音</span>
    </div>
  );
};
